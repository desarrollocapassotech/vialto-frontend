...
**Excepción a propósito — puntos de entrada a una función, no campos con un valor propio (sep 2026):** esta regla es para controles que *guardan un valor* que puede quedar inválido. No aplica a un botón que dispara una función que el tenant directamente no usa. `ExportarViajeModal.tsx`: el botón "NOMINA" (emite el documento PAUT) se oculta por completo — no se muestra deshabilitado con aviso — cuando el tenant tiene oculto el campo `paut` en su configuración de campos de Transportistas (`isHiddenAnywhere("paut")`, vía `useFieldConfig`), exactamente igual que ya se ocultaba por completo para viajes de flota propia (`permitePaut = !viajeUsaFlotaPropia(viaje) && !isHiddenAnywhere("paut")`). El campo oculto es una señal de que la empresa no trabaja con PAUT, no un dato que pueda quedar en un valor inconsistente a corregir — por eso ocultar el botón es correcto acá, no una violación de la regla de arriba.
...
### `CrudInput`/`CrudSelect` no llevan `w-full` propio — ojo al anidarlos fuera de un grid directo (sep 2026)

`baseClassName` de `CrudInput`/`CrudSelect` (`src/components/crud/CrudFields.tsx`) no incluye `w-full`. Funciona igual en el caso normal (`<label className="grid gap-1.5"><input .../></label>`) porque, como hijo **directo** de un contenedor `grid`, el input se estira solo al ancho de la columna (comportamiento default de CSS Grid, `justify-items: stretch`) — no hace falta la clase. Pero si el input queda anidado dentro de un `flex` intermedio (un componente compuesto como `TelefonoInput.tsx`, que envuelve `CrudSelect` + `CrudInput` en `flex flex-col sm:flex-row`), ese stretch automático de grid ya no llega — un `<input>` sin `width:100%` explícito vuelve a su ancho intrínseco de navegador (~200px) aunque su contenedor sea ancho. Bug real: el campo de teléfono en el alta/edición de Transportista quedaba angosto y desalineado del resto del formulario. Al construir un componente compuesto sobre `CrudInput`/`CrudSelect` que no sea hijo directo de un `grid`, pasar `className="w-full"` explícito (y `w-full` también al contenedor raíz del componente, no asumir que lo hereda).
...
### Conflicto de campo único (ID Fiscal/DNI) al importar Clientes/Transportistas/Choferes (sep 2026)

Cuando el preview detecta que una fila trae un ID Fiscal (Clientes/Transportistas) o DNI (Choferes) que ya pertenece a OTRO registro existente (`PreviewResult.advertenciasCampoUnicoDuplicado`, tipo `ImportCampoUnicoConflicto` en `types/api.ts` — ver `vialto-backend/CLAUDE.md`, sección `importaciones`), la resolución **no es un panel aparte** — va integrada directo en la card de esa fila dentro del listado "... en este archivo" (`FilaDetalleCard` en `ImportWizard.tsx`): badge ámbar "‹campoLabel› duplicado" en vez de "Nuevo"/"Actualiza", y debajo de los campos, dos botones **Ignorar fila** / **Actualizar ‹nombre existente›**. (Un panel separado que repetía la misma info de la fila se probó primero y se sacó por redundante — no reintroducirlo si se toca este flujo.)

El estado de las decisiones (`Record<fila, 'ignorar'|'actualizar'>`) vive en el componente `ImportWizard`, no en el hook — mismo criterio que `confirmarCamposFaltantes`/`confirmarFacturasDuplicadas`, se resetea en el mismo `useEffect` atado a `p?.sessionId`. `wizard.confirmarModuloActual()` manda `decisionesCampoUnicoDuplicado` al backend sin mirar qué módulo está activo (el mecanismo es genérico, no hardcodeado a Clientes — ver backend). El botón "Guardar y continuar" queda deshabilitado hasta que todas las filas en conflicto tengan una decisión elegida. El listado "... en este archivo" ya no tiene un `max-h`/`overflow-y-auto` propio (se sacó porque cortaba la vista de las tarjetas con conflicto) — crece libre, el scroll lo maneja la página.
...
## Fechas sin hora (vencimientos, fechas de carga, etc.): formatear y leer siempre en UTC

**Regla global — nunca uses getters de fecha locales (`getFullYear`/`getMonth`/`getDate`, ni `toLocaleDateString`/`Intl.DateTimeFormat` sin `timeZone: "UTC"`) sobre un campo que representa una fecha sin hora.**

El backend guarda estos campos (`vencimientoPoliza`, fechas de carga/descarga, etc.) como `DateTime` en medianoche UTC (`2026-05-15T00:00:00.000Z`). En cualquier navegador con huso horario detrás de UTC (Argentina, UTC-3), esa marca equivale a `2026-05-14T21:00:00` hora local — así que leerla con getters locales (`d.getDate()`) o formatearla con `toLocaleDateString` sin fijar `timeZone` muestra **un día antes** del valor real.

- **Para mostrar** (`fmtDate` de cada `ViewModal`): pasar siempre `timeZone: "UTC"` a `toLocaleDateString`/`Intl.DateTimeFormat` — patrón ya usado en Combustible (`combustibleExcelExport.ts`, `CombustibleDashboardPanels.tsx`), Mantenimiento (`FechaPicker.tsx`, `mantenimientoLabels.ts`) y Dashboard (`TenantOwnerDashboard.tsx`).
- **Para poblar un `<input type="date">`** (valor `YYYY-MM-DD` a partir de un ISO del backend): usar `getUTCFullYear()/getUTCMonth()/getUTCDate()`, nunca las variantes locales — ver `vehiculoVencimientoPolizaInputValue` en `lib/vehiculoForm.ts`.

Bug real corregido sep 2026: `VehiculoViewModal.tsx` (`fmtDate` sin `timeZone: "UTC"`) y `vehiculoForm.ts` (`vehiculoVencimientoPolizaInputValue` con getters locales) mostraban el vencimiento de póliza un día antes del real — reportado como "la fecha se importó mal" cuando en realidad el dato en base estaba correcto, solo la lectura en el frontend restaba un día. Antes de copiar el patrón `fmtDate` a una pantalla nueva, revisar si ya tiene `timeZone: "UTC"` — varios `ViewModal` (Cliente, Chofer, Transportista, Factura, etc.) definen su propio `fmtDate` duplicado y no todos fueron auditados todavía.
...
Última actualización: septiembre 2026 (conflicto de campo único ID Fiscal/DNI al importar Clientes/Transportistas/Choferes — resolución "ignorar/actualizar" integrada en la card de cada fila, en vez de un panel aparte, ver "Conflicto de campo único (ID Fiscal/DNI) al importar Clientes/Transportistas/Choferes"; de la misma pasada, la excepción a "advertir, no ocultar" para botones que disparan una función en vez de guardar un valor — botón NOMINA/PAUT oculto cuando el tenant tiene oculto el campo `paut`, ver "Controles incompatibles con el estado del tenant/registro: advertir, no ocultar"; y el gotcha de `CrudInput`/`CrudSelect` sin `w-full` propio al anidarlos fuera de un grid directo, ver "`CrudInput`/`CrudSelect` no llevan `w-full` propio..."; y, de una pasada anterior, fechas sin hora — reforzada la regla de formatear/leer siempre en UTC tras un bug real en `VehiculoViewModal.tsx`/`vehiculoForm.ts` que mostraba el vencimiento de póliza un día antes del real, ver "Fechas sin hora (vencimientos, fechas de carga, etc.): formatear y leer siempre en UTC"; y, de la misma pasada, facturas por tramo: el listado y el cobro leen `importe` + `ivaMonto` persistidos, no rearman el IVA en cada pantalla — ver `vialto-backend/CLAUDE.md`, sección "Facturas por tramo (LSF / Uruguay)"; y, de una pasada anterior, nueva pantalla `ConfiguracionNotificacionesTenantPage.tsx` en `/configuracion/notificaciones` — toggle on/off por tipo de notificación de email, contra `GET/POST /api/notificaciones/config[/toggle]` del backend; mismo patrón de guardado inmediato al tocar el switch que `CamposEmpresaPage.tsx`/`ConceptosConfigTenantPage.tsx`, sin patrón nuevo que documentar acá — ver `vialto-backend/CLAUDE.md`, sección "`notificaciones` — alertas por email vía Resend"; y, de una pasada anterior, `Viaje.precioTransportistaIvaIncluidoPct` — reemplaza al boolean v1, ya no hace falta el patrón "advertir, no ocultar" para este campo puntual pero la regla general sigue documentada, ver "Controles incompatibles..."; y, de una pasada anterior, wizard de importación masiva unificado en `ImportWizard.tsx` — páginas en vez de modal, selector de módulos, diff "Ver cambios" y demás patrones — ver "Importación masiva desde Excel"; y, de una pasada anterior a esa, el patrón de badges de estado aditivos para Facturas/Liquidaciones/Viajes — ver "Badges de estado: ejes aditivos, nunca se reemplazan")

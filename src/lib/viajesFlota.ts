...
export function mergeMaestroPorId<T extends { id: string }>(
  base: T[],
  extras: T[],
): T[] {
...
function stubTransportista(
  v: Viaje,
  t: { id: string; nombre: string },
): Transportista {
  return {
    id: t.id,
    tenantId: v.tenantId,
...
function stubChofer(
  v: Viaje,
  ch: Pick<Chofer, "id" | "nombre"> &
    Partial<Pick<Chofer, "dni" | "cuit" | "telefono" | "transportistaId">>,
): Chofer {
  return {
    id: ch.id,
    tenantId: v.tenantId,
...
export function entidadesMaestroStubsDesdeViaje(v: Viaje): {
  ...
  if (v.transportista)
    transportistas.push(stubTransportista(v, v.transportista));
 ...
 */
export function nombreClienteListadoViaje(
  v: Viaje,
  clientes?: Cliente[],
): string {
 ...
 */
export function monedaUnicaDeViajes(
  viajeIds: string[],
  viajes: Viaje[],
): string | null {

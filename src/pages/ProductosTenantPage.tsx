...
function PresentacionesPanel({
  ...
  const load = useCallback(async () => {
    try {
      const data = await apiJson<Presentacion[]>(
        `/api/stock/productos/${encodeURIComponent(producto.id)}/presentaciones`,
        () => getToken(),
      );
      setRows(data);
      setLoadError(null);
  ...
  async function submitForm() {
    ...
    if (!unidad.trim()) {
      setFormError('La unidad equivalente es obligatoria.');
      return;
    }
    const pesoNum = pesoKg.trim() ? parseFloat(pesoKg) : undefined;
    if (pesoKg.trim() && (isNaN(pesoNum!) || pesoNum! < 0)) {
    ...
    try {
      ...
      if (editRow) {
        await apiJson(
          `/api/stock/productos/${encodeURIComponent(producto.id)}/presentaciones/${encodeURIComponent(editRow.id)}`,
          () => getToken(),
  ...
  async function eliminar(p: Presentacion) {
    ...
    try {
      await apiJson(
        `/api/stock/productos/${encodeURIComponent(producto.id)}/presentaciones/${encodeURIComponent(p.id)}`,
        () => getToken(),
        { method: 'DELETE' },
      );
      await load();
...
function ProductoModal({
  ...
  async function submit() {
    ...
    try {
      ...
      if (modo === 'create') {
        await apiJson<Producto>('/api/stock/productos', () => getToken(), {
          method: 'POST',
          body: JSON.stringify(body),
        });
      } else {
        await apiJson<Producto>(
          `/api/stock/productos/${encodeURIComponent(productoInicial!.id)}`,
          () => getToken(),
          { method: 'PATCH', body: JSON.stringify(body) },
        );
      }

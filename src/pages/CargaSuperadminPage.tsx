...
export function CargaSuperadminPage() {
  ...
  const load = useCallback(async () => {
    ...
    const data = await apiJson<Paginated>(
      `/api/platform/cargas/paginated?${params.toString()}`,
      () => getToken(),
    );
    setRows(data.items);
    setMeta(data.meta);
  }, [getToken, tenantId, page, pageSize, qDebounced, filtroActivo]);
  ...
  useEffect(() => {
    setPage(1);
  }, [qDebounced, filtroActivo, tenantId]);
  ...
  async function submitEdit() {
    ...
    try {
      await apiJson<Carga>(
        `/api/platform/cargas/${encodeURIComponent(editId)}?tenantId=${encodeURIComponent(tenantId)}`,
        () => getToken(),
        {
          method: 'PATCH',
          body: JSON.stringify({
  ...
  async function toggleActivo(row: Carga) {
    if (
      row.activo &&
      !window.confirm(`¿Desactivar "${row.nombre}"? Los viajes que lo usan conservan el vínculo.`)
    ) {
      return;
    }
    setError(null);
    try {
      await apiJson<Carga>(
        `/api/platform/cargas/${encodeURIComponent(row.id)}?tenantId=${encodeURIComponent(tenantId)}`,
        () => getToken(),
  ...
  return (
    <div className="w-full">
      <h1 className="font-[family-name:var(--font-display)] text-4xl tracking-wide">Cargas</h1>
      <p className="mt-2 text-vialto-steel max-w-2xl">
        Catálogo de cargas transportables. Seleccioná una empresa para ver y gestionar sus cargas.
      </p>
      ...
      {!tenantId && tenants !== null && (
        <p className="mt-8 text-vialto-steel">Seleccioná una empresa para ver sus cargas.</p>
      )}
      ...
      {modalOpen && !editId && (
        <TipoCargaNuevoModal
          ...
          nombreInicial=""
          getToken={getToken}
          createPostUrl={`/api/platform/cargas?tenantId=${encodeURIComponent(tenantId)}`}
          onCreated={async () => {
            setModalOpen(false);
            await load();
          }}

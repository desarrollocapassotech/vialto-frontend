...
export function EmitirFacturaModal({
  ...
  }, [clienteInicial, configUrl, factura.clienteId, getToken, platform, tenantId]);
  ...
  const missingEmitFields = useMemo(
    () =>
      collectFacturaEmitMissingFields({
        emisor: arcaConfig,
        cliente: clienteDetalle ?? { nombre: null, direccion: null, idFiscal: null, condicionIva: null },
      }),
  ...
  async function handleEmitirArca() {
    ...
    if (!datosReady) {
      notifyEmitBlock('Cargando datos de emisión. Intentá de nuevo en un momento.');
      return;
    }
    if (yaAutorizada) {
    ...
    if (!lineasCheck.ok) {
      ...
      notifyEmitBlock(lineasCheck.message);
      return;
    }
    setLineasIncomplete([]);
    setError(null);
    ...
    try {
      ...
      const msg =
        err instanceof ApiError && err.status === 404 && err.message?.toLowerCase().includes('arca')
          ? err.message
        ...
        >
          ...
          <div className="flex justify-end gap-3">
            {factura.comprobanteUrl && (
              <button
                type="button"
                onClick={() => setPreviewComprobanteUrl(factura.comprobanteUrl!)}
          ...
          {step === 'revision' && (
            <div className="space-y-5">
              ...
              <section className="space-y-1">
                ...
                <Row label="Fecha de emisión" value={fmtDate(factura.fechaEmision)} />
              </section>

              {bloqueadoUsd && (
                <p className="text-xs text-amber-900 border border-amber-400/40 bg-amber-50 px-3 py-2" role="alert">
                  {MSG_ARCA_NO_FACTURA_USD}
                </p>
              )}
          ...
          {step === 'autorizada' && (
            <div className="space-y-5">
              <div className="border border-emerald-200 bg-emerald-50 px-4 py-3">
                ...
                {facturaEmitida.cae && (
                  <p className="text-xs text-emerald-700 mt-0.5">CAE: {facturaEmitida.cae}</p>
                )}
              ...
              <section className="space-y-1.5">
                ...
                <Row label="Cliente" value={clienteDetalle?.nombre ?? '—'} />

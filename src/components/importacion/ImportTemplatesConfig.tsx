...
/**
 * Configuración de templates de importación (mapeo de columnas por módulo) —
 * compartido entre la pantalla standalone (`SuperadminImportTemplatesPage`,
 * llegada desde el wizard de import o el link "Configurar templates") y una
 * sección embebida en "Configuración por empresa" (`CamposEmpresaPage`),
 * mismo patrón que `ArcaConfigTenantPage`/`ConfiguracionNotificacionesTenantPage`
 * — un solo componente, controlado por props, sin duplicar la lógica.
 */
export function ImportTemplatesConfig({
  tenantId,
  tenantNombre,
  tenantModules,
  initialModulo,
  ...
  /** Precarga este módulo al montar (ej. `?modulo=` desde el error de "columnas faltantes" del wizard). */
  initialModulo?: string | null;
  /** true = se está mostrando dentro de otra pantalla (Configuración por empresa) — oculta el título propio. */
  embedded?: boolean;
}) {
  ...
  // Si llegamos con un módulo precargado (ej. desde el error de "columnas
  // faltantes" del wizard de import), lo usa primero. Si no, precarga el

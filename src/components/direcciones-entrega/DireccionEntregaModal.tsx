...
export function DireccionEntregaModal({
  getToken,
  onClose,
  onSaved,
  tenantId,
  ...
  onClose: () => void;
  onSaved: (direccion: DireccionEntrega) => void;
  tenantId?: string;
  stacked?: boolean;
}) {
  ...
  async function submit() {
    ...
    try {
      const path = tenantId
        ? `/api/platform/direcciones-entrega${qs}`
        : '/api/direcciones-entrega';
      const result = await apiJson<DireccionEntrega>(path, () => getToken(), {
        method: 'POST',
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="direccion-entrega-modal-title"
        className="w-full max-w-md rounded border border-black/10 bg-white shadow-lg"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-black/10 px-4 py-3">
          <h2 id="direccion-entrega-modal-title" className="text-lg font-semibold text-vialto-charcoal">
            Nueva dirección / ruta
          </h2>
          <button
            type="button"
            onClick={onClose}

...
export function ProductoModal({
  ...
  const [pesoStr, setPesoStr] = useState<string>(
    productoInicial?.pesoUnitarioKg != null
      ? String(productoInicial.pesoUnitarioKg)
      : "",
  );
        ...
        >
          <div className="flex items-center justify-between border-b border-black/10 px-5 pt-5 pb-4">
            <h2 className="font-[family-name:var(--font-display)] text-xl tracking-wide">
              {modo === "create"
                ? "Nuevo producto"
                : (productoInicial?.nombre ?? "Producto")}
            </h2>
            <button
              type="button"
              onClick={onClose}
                ...
                {showStock && (
                  <>
                    <div>
                      ...
                      <p className="mt-1 text-sm">
                        {productoInicial?.pesoUnitarioKg ?? "—"}
                      </p>
                    </div>
                    <div>
                          ...
                          {productoInicial.productoPresentaciones.map((pp) => (
                            <li key={pp.id} className="text-sm">
                              ...
                              <span className="text-vialto-steel">
                                {" "}
                                — {pp.unidadesPorBulto} unidades dentro del
                                bulto
                              </span>
                            </li>
                          ))}
                  ...
                  <>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      {rows.map((row, idx) => (
                        <div
                          key={row._key}
                          className="grid grid-cols-[1fr_6rem_2rem] gap-2 items-start"
                        >
                          <SearchableEntitySelect<Presentacion>
                            items={presentaciones}
                            value={row.presentacionId}
                            onChange={(id) =>
                              updateRow(row._key, { presentacionId: id })
                            }
      ...
      {newPpRowIdx !== null && (
        <PresentacionFormModal
          ...
          onSaved={(p) => {
            ...
            setRows((prev) =>
              prev.map((row, i) =>
                i === idx ? { ...row, presentacionId: p.id } : row,
              ),
            );
            setNewPpRowIdx(null);
          }}

...
function getPresentaciones(
  ...
  return (
    productos.find((p) => p.id === productoId)?.productoPresentaciones ?? []
  );
}
    ...
    <>
        ...
        <div className="space-y-3">
          <div>
            <h2 className="text-base font-semibold text-vialto-charcoal">
              Productos{" "}
              <span className="ml-1 text-sm font-normal text-vialto-steel">
                ({rows.length})
              </span>
            </h2>
            <p className="text-xs text-vialto-steel mt-0.5">
              Completá los datos de cada producto. Podés agregar más al terminar
              cada uno.
            </p>
          </div>
          ...
          {rows.map((row, idx) => {
            ...
            return (
              <div
                key={row._key}
                className="bg-white rounded-lg border border-black/10 p-4 space-y-4"
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <SearchableEntitySelect<Producto>
                      ...
                      inputClassName={`${INPUT} ${
                        fieldErrors[`row_${idx}_productoId`]
                          ? "border-red-400"
                          : ""
                      }`}
                      onNuevo={
                        canCreateProducto
                          ? () => setNuevoProductoRowKey(row._key)
                          : undefined
                      }
                      onNuevoLabel="+ Nuevo producto"
                    />
                    <CrudFieldError
                      message={fieldErrors[`row_${idx}_productoId`]}
                    />
                    <StockActual
                  ...
                  <div className="space-y-1">
                    ...
                    <CrudFieldError
                      message={fieldErrors[`row_${idx}_presentacionId`]}
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      value={row.bultos}
                      onChange={(e) =>
                        onUpdateRow(row._key, { bultos: e.target.value })
                      }
                      className={`${INPUT} ${
                    ...
                    />
                    <CrudFieldError
                      message={fieldErrors[`row_${idx}_bultos`]}
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      value={row.sueltas}
                      onChange={(e) =>
                        onUpdateRow(row._key, { sueltas: e.target.value })
                      }
                      className={INPUT}
                      placeholder="0"
                    />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className={`${INPUT} ${
                        fieldErrors[`row_${idx}_fechaVencimiento`]
                          ? "border-red-400"
                          : ""
                      }`}
                    />
                    <CrudFieldError
                      message={fieldErrors[`row_${idx}_fechaVencimiento`]}
                    />
                  </div>
                </div>
              </div>
      ...
      {nuevoProductoRowKey !== null && (
        <ProductoModal
          ...
          onSaved={(p) => {
            onProductoCreado(p);
            onUpdateRow(nuevoProductoRowKey, {
              productoId: p.id,
              presentacionId: "",
            });
